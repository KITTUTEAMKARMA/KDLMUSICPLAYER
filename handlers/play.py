...
async def playlist(client, message):
    ...
    if not queue:
        await message.reply_text("❌ **first vc la song podu da butter biscut**")
    temp = []
    for t in queue:
        temp.append(t)
...
async def p_cb(b, cb):
    ...
    if type_ == "playlist":
        ...
        if not queue:
            await cb.message.edit("❌ **first vc la song podu da butter biscut**")
        temp = []
        for t in queue:
            temp.append(t)
        now_playing = temp[0][0]
...
async def play(_, message: Message):
        ...
        for administrator in administrators:
            if administrator == message.from_user.id:
                ...
                try:
                    ...
                    await lel.edit(
                        f"✅ **Thalivan vanthutan da eni attam pattam kondatam than**",
                    )
    ...
    if audio:
        ...
        try:
            ...
            await lel.edit(
                "😕 **ᴇᴠᴏʟᴏ ᴀɢᴀʟᴀ /ᴘʟᴀʏ ᴘᴏᴅᴀ ᴛʜᴇʀᴜɴᴊᴀ ᴜɴᴀᴋᴜ ᴀᴛʜᴜ ᴋᴜ ᴀᴘʀᴏᴍ sᴏɴɢ ᴛɪᴛʟᴇ ᴘᴏᴅᴀɴᴜᴍ ᴛʜᴇʀɪᴀᴍᴀ ᴘᴏᴄʜᴀ🤷‍♂️**\n\n» **please provide the correct song name or include the artist's name as well**"
            )
            print(str(e))
            return
        keyboard = InlineKeyboardMarkup(
        ...
        try:
            ...
            await lel.edit(
                "😕 **sᴏɴɢ ɴᴀᴍᴇ ᴏʟᴜɴɢᴀ ᴘᴏᴅᴜɢᴀ ᴘᴀ ᴋᴀᴅᴜᴘᴀ ᴇʀᴜᴋᴜ**\n\n» **please sᴏɴɢ ɴᴀᴍᴇ ᴏʟᴜɴɢᴀ ᴘᴏᴛᴀ ᴛʜᴀɴ ɴᴀɴ ᴘᴀᴅᴜᴠᴀɴ ᴇʟʟᴀ ɴᴀ ɴᴀɴ sɪʟᴇɴᴛ ᴀᴀ ᴛʜᴀɴ ᴇʀᴜᴘᴀɴ😏**"
            )
        # veez project
        try:
            toxxt = "\n"
    ...
    if chat_id in callsmusic.pytgcalls.active_calls:
        ...
        await message.reply_photo(
            ...
            + f"🎧 **entha song aa evaru than potaru:** {message.from_user.mention}",
            reply_markup=keyboard,
        )
        os.remove("final.png")
...
async def lol_cb(b, cb):
    ...
    try:
        ...
        if (dur / 60) > DURATION_LIMIT:
            await cb.message.edit(
                f"❌ **one hour kula podunga bro nan pavom 🥺** `{DURATION_LIMIT}` **minutes, can't play !**"
            )
            return
    except:
        pass
